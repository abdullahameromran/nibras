...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (requestedSlug) {
      const { data: existingSlug } = await admin.from("schools").select("id").eq("slug", baseSlug).maybeSingle()
      if (existingSlug) {
    ...
    // 1. Create the school
    const { data: school, error: schoolErr } = await admin.from("schools").insert({ name: school_name, slug: finalSlug, timezone }).select().single()
    if (schoolErr) {
...
async function generateUniqueSchoolSlug(baseSlug: string) {
  ...
  while (true) {
    const { data: existing, error } = await admin.from("schools").select("id").eq("slug", candidate).maybeSingle()
    if (error) {
      throw error
    }
    if (!existing) {
