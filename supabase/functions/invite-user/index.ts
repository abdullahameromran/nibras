...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { school_id, email, role, first_name, last_name, national_id } = body
    ...
    // both a teacher and a student, so reject that accidental role collision.
    const { data: existingSchoolRoles, error: existingRolesError } = await admin
      .from("user_school_roles")
      .select("role")
      .eq("user_id", userId)
      .eq("school_id", school_id)
      .eq("is_active", true)
    if (existingRolesError) return json({ error: existingRolesError.message }, 400)
    const conflictingRole = existingSchoolRoles?.find((entry) => entry.role !== role)?.role
    if (conflictingRole) {
      return json({ error: `This email already belongs to an active ${conflictingRole} account in this school.` }, 409)
    }
    ...
    if (profileError) {
      const message = profileError.code === "23505" ? "This National ID is already used by another student." : profileError.message
      return json({ error: message }, profileError.code === "23505" ? 409 : 400)
    }

    // Assign the role (idempotent thanks to the unique constraint)
    const { error: roleErr } = await admin.from("user_school_roles").upsert({ user_id: userId, school_id, role, is_active: true }, { onConflict: "user_id,school_id,role" })
