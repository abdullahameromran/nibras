...
/**
 * Authentication emails must always point to the public application, even when
 * the request is initiated while running the frontend locally.
 */
export const SITE_URL = "https://www.nibrasedtech.com"
...
export async function sendMagicLink(email: string): Promise<string | null> {
  const { error } = await supabase.auth.signInWithOtp({
    email,
    options: {
      // Use the exact allow-listed production origin. Appending a path or
      // trailing slash can fail when the hosted Auth redirect list is exact.
