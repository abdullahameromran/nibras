...
export function useTimetable(schoolId: string | null, filters?: {
  ...
  const fetchTimetable = useCallback(async () => {
    ...
    let query = supabase
      ...
      .select(`
        id, school_id, academic_year_id, working_day_id, time_slot_id,
...
export function useTimeSlots(schoolId: string | null) {
  ...
  const createTimeSlot = useCallback(async (slot: Omit<TimeSlot, "id">) => {
    const { data, error: err } = await supabase.from("time_slots").insert(slot).select("id, school_id, label, start_time, end_time, sort_order").single();
    if (err) return { error: err.message, data: null };
