...
export function useSchools() {
  ...
  const fetchSchools = useCallback(async () => {
    ...
    const [{ data, error: schoolsError }, { data: rolesData, error: rolesError }] = await Promise.all([
      supabase
        .from("schools")
        .select(`
          id, name, slug, logo_url, timezone, is_active, settings,
  ...
  const updateSchoolSubscription = useCallback(async (schoolId: string, planId: string) => {
    ...
    const query = current?.id
      ? supabase.from("school_subscriptions").update({ plan_id: planId }).eq("id", current.id)
      : supabase.from("school_subscriptions").insert({ school_id: schoolId, plan_id: planId, status: "trialing", starts_at: new Date().toISOString() });
  ...
  return { schools, loading, error, fetchSchools, createSchool, updateSchool, updateSchoolSubscription, activateSchool, deactivateSchool, deleteSchool };
}
