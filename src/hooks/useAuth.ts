...
function pickInitialRoleContext(roles: UserRoleRecord[]): Pick<AuthState, "activeRole" | "activeSchoolId"> {
  ...
  const storedMatch = stored ? roles.find((role) => role.role === stored.role && (role.school_id ?? null) === stored.schoolId) : null
...
export function useAuth(): UseAuthReturn {
  ...
  const signUpSchool = useCallback(
    async (payload: SchoolSignupPayload) => {
  ...
  return {
    ...
    signOut: doSignOut,
    setActiveRole,
    portal: state.loading ? "login" : state.user ? (state.activeRole ? roleToPortal(state.activeRole) : "login") : "login",
  }
}
