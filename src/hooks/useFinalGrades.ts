...
export function useFinalGrades(filters: {
  ...
  const fetchGrades = useCallback(async () => {
    ...
    let query = supabase
      .from("final_grades")
      .select(`
        id, school_id, academic_year_id, class_id, subject_id, student_id,
        grade_value, grade_letter, remarks, status, submitted_by, submitted_at,
        approved_by, approved_at, deleted_at,
        subjects ( id, name ),
        classes ( id, name ),
        profiles:profiles!final_grades_student_id_fkey ( id, first_name, last_name )
      `)
      .is("deleted_at", null)
  ...
  const upsertGrade = useCallback(async (grade: Partial<FinalGrade> & Pick<FinalGrade, "school_id" | "academic_year_id" | "class_id" | "subject_id" | "student_id">) => {
    const { data, error: err } = await supabase
      ...
      .select("id, school_id, academic_year_id, class_id, subject_id, student_id, grade_value, grade_letter, remarks, status, submitted_by, submitted_at, approved_by, approved_at, deleted_at")
      .single();
