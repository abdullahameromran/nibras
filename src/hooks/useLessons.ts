...
export function useLessons(filters: {
  ...
  const fetchLessons = useCallback(async () => {
    ...
    setError(null);
    let selectStr = `
      id, school_id, class_id, subject_id, teacher_id, title, description,
      video_url, lesson_date, deleted_at, created_at, updated_at,
      subjects ( id, name ),
      classes ( id, name ),
  ...
  useRealtimeRefresh(fetchLessons, ["lessons", "lesson_attachments", "student_lesson_progress"]);
  ...
  const createLesson = useCallback(async (lesson: {
    ...
    const { data, error: err } = await supabase
      .from("lessons")
      .insert(lesson)
      .select("id, school_id, class_id, subject_id, teacher_id, title, description, video_url, lesson_date, deleted_at, created_at, updated_at")
      .single();
