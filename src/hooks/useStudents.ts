...
type StudentProfileRow = Pick<
  Student,
  "id" | "email" | "first_name" | "last_name" | "avatar_url" | "phone" | "national_id" | "is_active"
>;
...
function buildStudentRecord(args: {
  ...
  return {
    ...
    national_id: args.profile?.national_id ?? null,
...
export function useStudents(schoolId: string | null, classId?: string | null) {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    const fetchProfilesByIds = async (ids: string[]) => {
      ...
      const { data, error: profilesError } = await supabase
        .from("profiles")
        .select("id, email, first_name, last_name, avatar_url, phone, national_id, is_active")
        .in("id", ids);
  ...
  const updateStudent = useCallback(async (userId: string, updates: { first_name: string; last_name?: string; phone?: string; national_id?: string; is_active: boolean; class_id?: string }) => {
    if (!schoolId) return { error: "No school selected" };
