...
export function useSchoolDetails(schoolId: string | null) {
  ...
  const fetchSchoolDetails = useCallback(async () => {
    ...
    const [{ data: schoolData, error: schoolError }, { data: subscriptionData, error: subscriptionError }] = await Promise.all([
      supabase
        .from("schools")
        .select("id, name, slug, logo_url, timezone, is_active, settings, created_at, updated_at")
        .eq("id", schoolId)
        .maybeSingle(),
      supabase
        .from("school_subscriptions")
        .select(`
          id, school_id, plan_id, status, starts_at, ends_at, external_ref, created_at, updated_at,
          subscription_plans (
            id,
            name,
            max_students,
