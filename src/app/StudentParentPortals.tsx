...
import { ParentChild, PortalTeacherAssignment, useClassTeacherAssignments, useParentChildren, useStudentEnrollment } from "@/hooks/usePortalContext"
...
function formatDate(value: string | null | undefined, includeYear = true) {
  ...
  return new Date(value).toLocaleDateString(undefined, includeYear ? { month: "short", day: "numeric", year: "numeric" } : { month: "short", day: "numeric" })
}
...
function isAnnouncementVisible(announcement: Announcement, role: "student" | "parent", classId: string | null, gradeLevelId: string | null) {
...
function buildCourseSummaries(lessons: Lesson[], homework: Homework[], tests: MonthlyTest[], finalGrades: FinalGrade[], assignments: PortalTeacherAssignment[]) {
  ...
  const ensureCourse = (subjectId: string | null, name: string) => {
    ...
    if (!course) {
      const assignment = assignments.find((row) => (subjectId && row.subject_id === subjectId) || row.subjects?.name === name)
      course = {
        key,
        subjectId,
        name,
...
function buildContacts(assignments: PortalTeacherAssignment[], conversations: Conversation[]) {
            ...
            {contacts.map((contact) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  {contact.unreadCount > 0 && <span className="min-w-5 h-5 px-1 rounded-full bg-primary text-white text-[10px] font-bold flex items-center justify-center">{contact.unreadCount}</span>}
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="max-w-3xl mx-auto bg-card rounded-2xl border border-border shadow-sm p-6 space-y-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {language === "ar"
              ? `تمت الإجابة عن ${Object.keys(answers).length} من ${normalizedQuestions.length} سؤال.`
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {normalizedQuestions.map((item, index) => (
            ...
            >
              {t("Question")} {index + 1}: {t(answers[item.id] ? "Answered" : "Missing")}
            </button>
          ))}
        </div>
        <div className="flex justify-between gap-3">
          <Btn variant="secondary" onClick={() => setReviewMode(false)}>
            {t("Back to questions")}
          </Btn>
...
export function StudentPortal({ view, setView, onLogout, schoolId, user }: StudentPortalProps) {
  ...
  const announcements = announcementsQuery.announcements.filter((item) => isAnnouncementVisible(item, "student", classId, gradeLevelId))
  const courses = buildCourseSummaries(lessonsQuery.lessons, homeworkQuery.homework, testsQuery.tests, gradesQuery.grades, assignmentsQuery.assignments)
  ...
  const selectedLessonVideoSource =
    ...
    selectedLesson?.lesson_attachments?.find((attachment) => attachment.file_kind?.toLowerCase().includes("video") || /\.(mp4|webm|ogg|mov|m4v)(?:[?#].*)?$/i.test(attachment.file_url))?.file_url ??
    null
  ...
  const finalGradeScores = gradesQuery.grades.map((item) => item.grade_value).filter((value): value is number => value != null)
  const submissionScores = [
    ...homeworkQuery.homework.map((item) => getHomeworkSubmission(item, userId)?.score).filter((value): value is number => value != null),
    ...testsQuery.tests.map((item) => getTestSubmission(item, userId)?.score).filter((value): value is number => value != null),
  ]
  ...
  const attendanceRate = attendanceQuery.records.length > 0 ? Math.round((attendancePresent / attendanceQuery.records.length) * 100) : 0
  ...
  useEffect(() => {
    if (view !== "messages" || !activeConversation || !userId) return
    const unreadMessages = activeConversation.messages.filter(
      (message) => message.sender_id !== userId && (message.message_recipients ?? []).some((recipient) => recipient.recipient_id === userId && !recipient.is_read),
    )
    if (unreadMessages.length === 0) return
    Promise.all(unreadMessages.map((message) => messagesQuery.markAsRead(message.id))).catch(() => null)
  }, [activeConversation, messagesQuery.markAsRead, userId, view])
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      `nibras-live-homework-${activeHomeworkId}`,
      JSON.stringify({
  ...
  const studentRole = enrollmentQuery.enrollment?.classes?.name ? `${t("Student")} - ${enrollmentQuery.enrollment.classes.name}` : t("Student")
  ...
  return (
    <>
      <AppShell
        ...
        navBadges={{ messages: messagesQuery.totalUnread }}
        activeView={view}
        onSelect={setView}
        onLogout={onLogout}
        headerTitle={t(({
        ...
        userId={userId}
      >
        {!classId && <EmptyState title="No active enrollment found" description="This student account is connected, but there is no active class enrollment yet in Supabase." />}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {courses.map((course) => {
              const averageScore = average([
                ...course.homework.map((item) => getHomeworkSubmission(item, userId)?.score).filter((value): value is number => value != null),
                ...course.tests.map((item) => getTestSubmission(item, userId)?.score).filter((value): value is number => value != null),
              ])
              return (
                <button
                  key={course.key}
          ...
          <div className="space-y-5">
            ...
            {courseTab === "lessons" && (
              <div className="space-y-4">
                ...
                {selectedCourse.lessons.length === 0 && <EmptyState title="No lessons published" description="Lessons created for this subject will appear here." />}
                {selectedLesson && (
                      ...
                      <div className="flex gap-3">
                        {selectedLessonVideoUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-secondary text-primary text-sm font-semibold hover:brightness-95"
                          >
                            <PlayCircle className="w-4 h-4" />
                            Watch video
                          </a>
                        )}
                        {!selectedLesson.student_lesson_progress?.some((row) => Boolean(row.completed_at)) && (
            ...
            {courseTab === "homework" && (
              <div className="space-y-4">
                ...
                {selectedCourse.homework.length === 0 && <EmptyState title="No homework assigned" description="Homework created in Supabase for this subject will appear here." />}
              </div>
            )}
            ...
            {courseTab === "tests" && (
              <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            {formatDate(item.test_date)} - {item.test_questions?.length ?? 0} questions
                          </p>
                        </div>
                        {submission ? (
                          <div className="text-right shrink-0">
                ...
                {selectedCourse.tests.length === 0 && <EmptyState title="No tests scheduled" description="Monthly tests for this subject will appear here once they are published." />}
              </div>
            )}
                  ...
                  <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Status: {selectedCourse.finalGrade.status}</p>
                      </div>
        ...
        {classId && !activeTest && !activeHomework && view === "tests" && (
          <div className="space-y-5">
            ...
            <div className="space-y-4">
                  ...
                  <div key={item.id} className="bg-card rounded-2xl border border-border shadow-sm p-5">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {submission ? (
                        ...
                        <Btn icon={<PlayCircle className="w-4 h-4" />} onClick={() => handleStartTest(item.id)}>
                          Start test
                        </Btn>
                      )}
                    </div>
              ...
              {filteredTests.length === 0 && <EmptyState title="No tests found" description="Try another course or month filter, or publish a new test in Supabase." />}
            </div>
          </div>
        )}
        ...
        {classId && !activeTest && !activeHomework && view === "announcements" && (
          <div className="space-y-4">
            ...
            {announcements.length === 0 && <EmptyState title="No announcements yet" description="Published announcements targeted to this student will appear here." />}
          </div>
        )}
...
export function ParentPortal({ view, setView, onLogout, schoolId, user }: ParentPortalProps) {
  ...
  const testsQuery = useTests({ schoolId, classId: selectedChild?.class_id ?? null, studentId: selectedChild?.student_id ?? null })
  ...
  const announcements = announcementsQuery.announcements.filter((item) => isAnnouncementVisible(item, "parent", selectedChild?.class_id ?? null, selectedChild?.grade_level_id ?? null))
  ...
  const parentResultRecords = [
    ...childHomework.flatMap((entry) => entry.submission?.score == null ? [] : [{
      id: `homework-${entry.submission.id}`,
      subjectId: entry.item.lessons?.subject_id ?? "homework",
      subjectName: entry.item.lessons?.subjects?.name ?? t("Course"),
      title: entry.item.title,
      type: t("Homework"),
      score: Number(entry.submission.score),
      date: entry.submission.submitted_at,
    }]),
    ...childTests.flatMap((entry) => entry.submission?.score == null ? [] : [{
      id: `test-${entry.submission.id}`,
      subjectId: entry.item.subject_id,
      subjectName: entry.item.subjects?.name ?? t("Course"),
      title: entry.item.title,
      type: t("Test"),
      score: Number(entry.submission.score),
      date: entry.submission.submitted_at,
    }]),
    ...gradesQuery.grades.flatMap((item) => item.grade_value == null ? [] : [{
      id: `final-${item.id}`,
      subjectId: item.subject_id,
      subjectName: item.subjects?.name ?? t("Course"),
      title: t("Final Grade"),
      type: t("Final Grade"),
      score: Number(item.grade_value),
      date: item.approved_at ?? item.submitted_at ?? "",
    }]),
  ].sort((left, right) => new Date(right.date).getTime() - new Date(left.date).getTime())
  const subjectPerformance = Array.from(parentResultRecords.reduce((result, record) => {
    const current = result.get(record.subjectId) ?? { id: record.subjectId, name: record.subjectName, total: 0, count: 0 }
    current.total += record.score
    current.count += 1
    result.set(record.subjectId, current)
    return result
  }, new Map<string, { id: string; name: string; total: number; count: number }>()).values()).map((item) => ({
    ...item,
  ...
  return (
    <>
      <AppShell
        ...
        userId={userId}
      >
        {childrenQuery.children.length === 0 && <EmptyState title="No linked children found" description="This parent account is connected, but there are no child records linked in Supabase yet." />}
          ...
          <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                  ...
                  <div className="space-y-4">
                    <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                      ...
                      <div className="space-y-3">
                        {subjectPerformance.slice(0, 4).map((item) => {
                        ...
                        {subjectPerformance.length === 0 && <p className="text-sm text-muted-foreground">{t("No scored work yet")}</p>}
                      </div>
                    </div>
                    <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
            ...
            {view === "progress" && (
              <div className="space-y-5">
                ...
                <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                  ...
                  <div className="space-y-4">
                    {parentResultRecords.map((item) => {
                      const score = item.score
                      const gradeLabel = scoreToLetter(score)
                      return (
                        <div key={item.id} className="flex items-center gap-4">
                          <div className="w-48 shrink-0">
                            <p className="text-sm font-semibold text-foreground">{item.subjectName}</p>
                            <p className="text-xs text-muted-foreground">{item.type} • {item.title}</p>
                          </div>
                    ...
                    {parentResultRecords.length === 0 && <EmptyState title="No scored work yet" description="Homework, test, and final-grade results for this child will appear here." />}
                  </div>
                </div>
              </div>
            )}
            ...
            {view === "homework" && (
              <div className="space-y-4">
                ...
                {childHomework.length === 0 && <EmptyState title="No homework records" description="Homework and submission scores from Supabase will appear here." />}
              </div>
            )}
            ...
            {view === "tests" && (
              <div className="space-y-4">
                ...
                {childTests.length === 0 && <EmptyState title="No test records" description="Monthly tests and child submissions from Supabase will appear here." />}
              </div>
            )}
            ...
            {view === "announcements" && (
              <div className="space-y-4">
                ...
                {announcements.length === 0 && <EmptyState title="No announcements yet" description="Parent-targeted announcements from Supabase will appear here." />}
              </div>
            )}
