...
function TeacherClassesSection({ showToast }: { showToast: (msg: string, type?: "success" | "error") => void }) {
  ...
  const handleAddLesson = () => {
    if (!lessonName.trim()) {
      showToast("Lesson name is required", "error")
      return
    }
    setLessons((prev) =>
      prev.map((c) =>
        c.id === selectedClass?.id
          ? {
              ...c,
              lessons: c.lessons + 1,
              lessonList: [...c.lessonList, { id: c.lessonList.length + 1, title: lessonName, date: "Apr 18, 2024", type: lessonPdf ? "pdf" : lessonImage ? "image" : "video", hasHomework: false }],
            }
          : c,
      ),
    )
      ...
      setSelectedClass((prev) =>
        prev
          ? {
              ...
              lessonList: [
                ...prev.lessonList,
                { id: prev.lessonList.length + 1, title: lessonName, date: "Apr 18, 2024", type: lessonPdf ? "pdf" : lessonImage ? "image" : "video", hasHomework: false },
              ],
            }
          : null,
      )
  ...
  const saveEdit = () => {
    ...
    if (editTarget.kind === "lesson") {
      setLessons((prev) =>
        prev.map((cls) =>
          cls.id === selectedClass?.id ? { ...cls, lessonList: cls.lessonList.map((lesson) => (lesson.id === editTarget.id ? { ...lesson, title: editTitle.trim() } : lesson)) } : cls,
        ),
      )
  ...
  const confirmManageAction = () => {
    ...
    if (action === "archive") {
      ...
      if (kind === "lesson") {
        setLessons((prev) =>
          prev.map((cls) => (cls.id === selectedClass?.id ? { ...cls, lessonList: cls.lessonList.filter((lesson) => lesson.id !== id), lessons: Math.max(0, cls.lessons - 1) } : cls)),
        )
        setSubView("detail")
  ...
  const managementDialogs = (
    <>
      {editTarget && (
        <Modal title={`Edit ${editTarget.kind === "lesson" ? "Lesson" : editTarget.kind === "homework" ? "Homework" : "Task"}`} onClose={() => setEditTarget(null)}>
      ...
      {confirmTarget && (
        <Modal title={confirmTarget.action === "delete" ? "Delete item" : "Archive item"} onClose={() => setConfirmTarget(null)}>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {confirmTarget.action === "delete"
                ? `Delete “${confirmTarget.title}” permanently? This cannot be undone.`
                : `Archive “${confirmTarget.title}”? It will be hidden from the active lesson list.`}
            </p>
            <div className="flex gap-3">
              <Btn onClick={confirmManageAction} className={`flex-1 ${confirmTarget.action === "delete" ? "!bg-red-500 hover:!bg-red-600" : ""}`}>
                {confirmTarget.action === "delete" ? "Delete" : "Archive"}
              </Btn>
              <Btn variant="secondary" onClick={() => setConfirmTarget(null)}>
                Cancel
              </Btn>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" style={{ fontFamily: "'Poppins', sans-serif" }}>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="space-y-2">
            <label className="text-[15px] font-medium text-[#0E1B4A]">
              Lesson Name <span className="text-red-500 text-xs">*</span>
            </label>
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="w-14 h-14 flex items-center justify-center">
                    <svg viewBox="0 0 52 47" fill="none" className="w-14 h-12 text-[#955AC3]">
                      <path
                        d="M34.667 31.333l-8.667-8.666-8.667 8.666M26 22.667V46M44.2 39.987A11.667 11.667 0 0038.333 18H35.94A18.667 18.667 0 102 36.787"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file && file.size > 20 * 1024 * 1024) {
                      showToast("PDF must be smaller than 20MB", "error")
                      return
                    }
                    setLessonPdf(file)
                  }}
          ...
          <div className="flex gap-4 pt-4">
            <button onClick={handleAddLesson} className="px-8 py-2.5 rounded-lg text-white text-sm font-medium transition-colors" style={{ background: "rgba(149,90,195,0.85)" }}>
              Save Lesson
            </button>
            <button onClick={() => setSubView("detail")} className="px-8 py-2.5 rounded-lg border border-gray-200 text-[#344054] text-sm font-medium hover:bg-gray-50 transition-colors">
              Cancel
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#999] mt-0.5">
                {selectedLesson?.title} · {selectedClass?.name}
              </p>
            </div>
          </div>
          <div className="flex gap-3">
                ...
                <thead>
                  <tr className="bg-[#F7F4FF]">
                    {["#", "Student", "HW Score", "Test Score", "Grade"].map((h) => (
                      <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-[#0E1B4A]">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {HW_GRADES.map((s) => (
                    <tr key={s.rank} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${s.grade.startsWith("A") ? "bg-emerald-100 text-emerald-700" : s.grade.startsWith("B") ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {s.grade}
                        </span>
                      </td>
        ...
        <div className="space-y-5" style={{ fontFamily: "'Poppins', sans-serif" }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-[#999] mt-0.5">
                {selectedClass?.name} · {selectedLesson.date}
              </p>
            </div>
            <button
              onClick={() => setSubView("hw-grades")}
              ...
              <div className="col-span-8 space-y-5">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="bg-gradient-to-br from-[#F5F0FF] to-[#EDE9FE] h-48 flex items-center justify-center relative">
                    {selectedLesson.type === "video" ? (
                      <div className="flex flex-col items-center gap-2">
                        <PlayCircle className="w-12 h-12 text-[#955AC3]" />
                        <p className="text-[#955AC3] font-semibold text-sm">Video Lesson</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-2">
                        <FileText className="w-12 h-12 text-[#955AC3]" />
                        <p className="text-[#955AC3] font-semibold text-sm">PDF Document</p>
                      </div>
                    )}
                  ...
                  <div className="p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-[#0E1B4A]">{selectedLesson.title}</p>
                      <p className="text-xs text-[#999]">Uploaded {selectedLesson.date}</p>
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
                  <h3 className="text-[15px] font-semibold text-[#0E1B4A] flex items-center gap-2">
                    <Video className="w-4 h-4 text-[#955AC3]" /> Video Link
                  </h3>
                  <div className="flex gap-3">
                    <input
                      value={videoUrl}
                    ...
                    />
                    <button onClick={() => showToast("Video link saved")} className="px-5 py-2.5 rounded-lg text-white text-sm font-medium transition-colors" style={{ background: "#955AC3" }}>
                      Save
                    </button>
                  </div>
                  {videoUrl && (
                    <div className="flex items-center gap-2 p-3 bg-[#F5F0FF] rounded-lg">
                      ...
                      <a href={videoUrl} target="_blank" rel="noreferrer" className="text-xs text-[#955AC3] font-medium truncate hover:underline">
                        {videoUrl}
                      </a>
                    </div>
                  )}
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
                  <h3 className="text-[15px] font-semibold text-[#0E1B4A] flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#955AC3]" /> PDF Materials
                  </h3>
                  <div className="border border-dashed border-[#955AC3]/40 rounded-xl p-8 flex flex-col items-center gap-3 cursor-pointer hover:bg-[#955AC3]/5 transition-colors">
                    ...
                    <button onClick={() => showToast("File picker opened")} className="px-6 py-2 rounded-lg text-white text-xs font-medium mt-1" style={{ background: "#955AC3" }}>
                      Choose File
                    </button>
                  </div>
                </div>
          ...
          {(lessonTab === "homework" || lessonTab === "tasks") && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setShowArchived((value) => !value)} className="px-3 py-2.5 rounded-lg border border-gray-200 text-xs font-semibold text-[#59456B] hover:bg-gray-50">
                    {showArchived ? "Active items" : "Archived items"}
                  </button>
                  <button
                    onClick={() => setShowPublish(lessonTab === "homework" ? "homework" : "task")}
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            title="Edit"
                            onClick={() => startEdit({ kind: lessonTab === "homework" ? "homework" : "task", id: item.id, title: item.title })}
                          ...
                          <button
                            title="Archive"
                            onClick={() => setConfirmTarget({ kind: lessonTab === "homework" ? "homework" : "task", id: item.id, title: item.title, action: "archive" })}
                          ...
                          <button
                            title="Delete"
                            onClick={() => setConfirmTarget({ kind: lessonTab === "homework" ? "homework" : "task", id: item.id, title: item.title, action: "delete" })}
                  ...
                  <table className="w-full min-w-[620px]">
                    ...
                    <tbody>
                      ...
                      {[{ rank: 7, name: "Zara Okonkwo" }].map((s) => (
                        <tr key={s.rank} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                          <td className="px-5 py-3.5">
                            <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-400">{s.rank}</div>
                          </td>
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-3">
                              <Avatar name={s.name} size="sm" />
                          ...
                          <td className="px-5 py-3.5">
                            <span className="text-[10px] px-2.5 py-1 rounded-full font-semibold bg-red-100 text-red-600">Missing</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {/* Publish modal */}
          {showPublish && <PublishQuestionModal mode={showPublish} onClose={() => setShowPublish(null)} onPublish={showPublish === "homework" ? addHW : addTask} />}
        </div>
        {managementDialogs}
      </>
    )
      ...
      <div className="space-y-5" style={{ fontFamily: "'Poppins', sans-serif" }}>
        ...
        <div className="grid grid-cols-12 gap-5">
          ...
          <div className="col-span-7 space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {cls.lessonList
                  .filter((lesson) => !archivedLessons.includes(lesson.id))
                  .map((lesson, i) => (
                    <div
                      key={lesson.id}
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: `${cls.color}18` }}>
                        {lesson.type === "pdf" ? (
                          ...
                          <PlayCircle className="w-4 h-4" style={{ color: cls.color }} />
                        ) : (
                          <BookOpen className="w-4 h-4" style={{ color: cls.color }} />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-[#0E1B4A] truncate">
                          Lesson {i + 1}: {lesson.title}
                        </p>
                        <p className="text-xs text-[#999] mt-0.5">{lesson.date}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {lesson.hasHomework && <span className="text-[10px] bg-[#F5F0FF] text-[#955AC3] font-semibold px-2 py-0.5 rounded-full">HW</span>}
                        <button
                          title="Edit lesson"
                          className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all"
                          onClick={(e) => {
                            e.stopPropagation()
                            setSelectedLesson(lesson)
                            setSubView("lesson")
                            startEdit({ kind: "lesson", id: lesson.id, title: lesson.title })
                          }}
                        >
                        ...
                        <button
                          ...
                          className="w-7 h-7 rounded-lg hover:bg-amber-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all"
                          onClick={(e) => {
                            e.stopPropagation()
                            setSelectedLesson(lesson)
                            setSubView("lesson")
                            setConfirmTarget({ kind: "lesson", id: lesson.id, title: lesson.title, action: "archive" })
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            setSelectedLesson(lesson)
                            setSubView("lesson")
                            setConfirmTarget({ kind: "lesson", id: lesson.id, title: lesson.title, action: "delete" })
                          }}
                        >
                          <Trash2 className="w-3.5 h-3.5 text-red-400" />
                        </button>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  className="text-xs text-[#955AC3] font-medium hover:underline"
                  onClick={() => {
                    setSelectedLesson(cls.lessonList[0])
                    setSubView("hw-grades")
                  }}
                >
                  View Grades
                </button>
          ...
          <div className="col-span-5 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[15px] font-semibold text-[#0E1B4A]">
              ...
              <div className="grid grid-cols-7 mb-2">
                {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((d) => (
                  <div key={d} className="text-center text-[11px] font-semibold text-[#999] py-1">
                    {d}
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-7 gap-y-1">
                ...
                {calDays.map((d) => {
                  ...
                  return (
                    <div
                      key={d}
                      className={`flex items-center justify-center h-7 w-7 mx-auto rounded-full text-[12px] cursor-pointer transition-colors font-medium
                      ${isToday ? "bg-[#955AC3] text-white" : isLesson ? "bg-[#F5F0FF] text-[#955AC3]" : "text-[#0E1B4A] hover:bg-gray-100"}`}
                    >
                      {d}
                    </div>
                  )
              ...
              <div className="flex gap-4 mt-3 pt-3 border-t border-gray-100">
                <div className="flex items-center gap-1.5 text-xs text-[#999]">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#955AC3]" />
                  Today
                </div>
                <div className="flex items-center gap-1.5 text-xs text-[#999]">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#F5F0FF]" />
                  Lesson day
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                {[
                  ...
                  <div key={s.label} className="flex items-center gap-2.5 rounded-xl p-3" style={{ background: `${cls.color}0A` }}>
                    ...
                    <div>
                      <p className="text-[18px] font-bold leading-none" style={{ color: cls.color }}>
                        {s.value}
                      </p>
                      <p className="text-[10px] font-medium mt-0.5" style={{ color: cls.color }}>
                        {s.label}
                      </p>
                    </div>
                  </div>
...
export function TeacherPortal({ view, setView, onLogout, schoolId, user }: { view: string; setView: (v: string) => void; onLogout: () => void; schoolId?: string | null; user?: any }) {
  ...
  const uniqueCourseNames = Array.from(new Set([...dbLessons.lessons.map((lesson) => lesson.subjects?.name).filter(Boolean), ...dbTests.tests.map((test) => test.subjects?.name).filter(Boolean)]))
  ...
  return (
    <>
      <AppShell navItems={TEACHER_NAV} activeView={view} onSelect={setView} onLogout={onLogout} headerTitle={titleMap[view] || "Dashboard"} userName="Mrs. Fatima Bello" userRole="Mathematics Teacher">
            ...
            <div className="grid grid-cols-12 gap-5">
              ...
              <div className="col-span-4 space-y-4">
                ...
                <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                  ...
                  <div className="space-y-3">
                    ...
                    {topStudents.length === 0 && <p className="text-xs text-muted-foreground">No scored submissions yet.</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="col-span-3 space-y-4">
                ...
                <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                  ...
                  <div className="space-y-3">
                    {[
                      { d: "31", t: "Meeting with the…", sub: "Meeting the Parents committee", over: true },
                      { d: "04", t: "Meeting with the…", sub: "Staff meeting — room 12" },
                      { d: "12", t: "Class B middle s…", sub: "Mid-term test" },
                      { d: "18", t: "Send Mr Ayo cla…", sub: "Send class notes" },
                    ].map((a, i) => (
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button className="text-xs text-primary font-semibold" onClick={() => setView("messages")}>
                    View All
                  </button>
                </div>
                <div className="space-y-4">
              ...
              <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  {classProgress.length === 0 && <p className="text-xs text-muted-foreground">Class averages will appear after students submit homework or tests.</p>}
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          <Users className="w-3 h-3" />
                          {t.submissions}/{t.total} submitted
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {!t.published && (
                        <Btn
                          size="sm"
                          onClick={() => {
                            setTests((prev) => prev.map((test) => (test.id === t.id ? { ...test, published: true } : test)))
                            showToast("Test published! Students notified.")
                          }}
                        >
                          Publish
                        </Btn>
                      ...
                      {t.published && (
                        <Btn size="sm" variant="secondary" icon={<Eye className="w-3 h-3" />} onClick={() => setShowTestResults(t.id)}>
                          View Results
                        </Btn>
                      )}
                    </div>
        ...
        {view === "tests" && showTestResults !== null && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Submissions", value: "22/25", color: "#10B981" },
                { label: "Class Avg", value: "74%", color: "#7C5CBF" },
                { label: "Top Score", value: "98%", color: "#F59E0B" },
              ].map((s) => (
                    ...
                    {HW_GRADES.map((s) => {
                      ...
                      return (
                        <tr key={s.rank} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className="text-sm font-bold" style={{ color: gpa >= 3.5 ? "#10B981" : gpa >= 3.0 ? "#7C5CBF" : "#F59E0B" }}>
                              {gpa.toFixed(1)}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                    ...
                    {HW_GRADES.map((s) => {
                      ...
                      return (
                        <tr key={s.rank} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <Badge color={s.grade.startsWith("A") ? "green" : s.grade.startsWith("B") ? "blue" : "gray"}>{s.grade}</Badge>
                          </td>
                        </tr>
                      )
                    })}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              {STUDENTS.map((s) => (
                <button
                  ...
                  className="bg-card rounded-2xl p-4 border border-border shadow-sm flex items-center gap-4 text-left hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-md transition-all"
                >
                        ...
                        <td key={di} className="px-4 py-3">
                          {days[i] === "Break" ? (
                            ...
                            <div className={`rounded-lg px-2 py-1.5 ${days[i] === "Mathematics" ? "bg-purple-50" : "bg-blue-50"}`}>
                              <p className={`text-xs font-semibold ${days[i] === "Mathematics" ? "text-primary" : "text-blue-600"}`}>{t(days[i])}</p>
                            </div>
                          )}
                        </td>
                      ))}
        ...
        {view === "classes" && <TeacherClassesSectionLive schoolId={schoolId ?? null} teacherId={user?.id ?? null} />}
      </AppShell>
      ...
      {selectedStudentProfile && (
        <Modal title={selectedStudentProfile.name} onClose={() => setSelectedStudentProfile(null)}>
          <div className="space-y-5">
            <div className="flex items-center gap-4 rounded-2xl bg-secondary/50 p-4">
              <Avatar name={selectedStudentProfile.name} size="lg" />
              <div>
                <p className="font-bold text-foreground">{selectedStudentProfile.name}</p>
                <p className="text-sm text-muted-foreground">
                  {selectedStudentProfile.class} · {selectedStudentProfile.email}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">Parent: {selectedStudentProfile.parent}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl bg-muted p-3 text-center">
                <p className="text-lg font-bold text-primary">{selectedStudentProfile.grade}</p>
                <p className="text-xs text-muted-foreground">Current grade</p>
              </div>
              <div className="rounded-xl bg-muted p-3 text-center">
                <p className="text-lg font-bold text-foreground">{selectedStudentProfile.points}</p>
            ...
            <div className="rounded-xl border border-border p-4 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">Homework completion 88% · Monthly-test average 84% · Last activity today.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Btn
                icon={<MessageSquare className="w-4 h-4" />}
                onClick={() => {
                  setMessageTarget({ name: selectedStudentProfile.name, relationship: "student" })
                  setMessageDraft("")
                }}
              >
                Message Student
              ...
              <Btn
                ...
                onClick={() => {
                  setMessageTarget({ name: selectedStudentProfile.parent, relationship: "parent" })
                  setMessageDraft("")
                }}
              >
                Message Parent
      ...
      {messageTarget && (
        <Modal title={`Message ${messageTarget.name}`} onClose={() => setMessageTarget(null)}>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Sending to the student's {messageTarget.relationship}.</p>
            <textarea
              value={messageDraft}
              onChange={(event) => setMessageDraft(event.target.value)}
              rows={4}
              placeholder="Write your message…"
              className="w-full rounded-xl border border-border bg-muted px-3 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30"
            />
            <div className="flex gap-3">
              <Btn
                className="flex-1"
              ...
              <Btn variant="secondary" onClick={() => setMessageTarget(null)}>
                Cancel
              </Btn>
            </div>
          </div>
