...
const SchoolAdminPortal = lazy(() => import("./SchoolAdminPortalLive").then((module) => ({ default: module.SchoolAdminPortalLive })))
const TeacherPortal = lazy(() => import("./TeacherPortalLive").then((module) => ({ default: module.TeacherPortalLive })))
const StudentPortalLive = lazy(() => import("./StudentParentPortals").then((module) => ({ default: module.StudentPortal })))
const ParentPortalLive = lazy(() => import("./StudentParentPortals").then((module) => ({ default: module.ParentPortal })))
...
function LoginPage({ onLogin, onRequestPasswordReset }: { onLogin: (email: string, password: string) => Promise<string | null>; onRequestPasswordReset: (email: string) => Promise<string | null> }) {
  ...
  return (
    <div
      ...
      style={{ fontFamily: language === "ar" ? "Tahoma, Arial, sans-serif" : "'Poppins', sans-serif" }}
    >
        ...
        <div className="flex items-center justify-between px-8 pt-6 pb-0">
          ...
          <p className="text-[14px] text-[#0e1b4a]">
            ...
            <button
              ...
              className="text-[#955ac3] font-medium hover:underline"
            >
              Sign Up
            </button>
          </p>
          ...
          <form onSubmit={handleSubmit} className="w-[333px]">
            ...
            <div className="mb-4">
              ...
              <div className="relative w-full h-[44px] rounded-[8px]" style={{ background: "rgba(214,186,234,0.25)" }}>
                ...
                />
                <button type="button" onClick={() => setShowPassword((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#955ac3] opacity-60 hover:opacity-100 transition-opacity">
            ...
            <div className="flex items-center justify-between w-full mb-10">
              <button type="button" onClick={() => setKeepLoggedIn((p) => !p)} className="flex items-center gap-1.5 cursor-pointer">
                <div
              ...
              <button
                ...
                className="text-[12px] font-medium text-[#955ac3] opacity-70 hover:opacity-100 transition-opacity"
              >
                Forgot password
              </button>
            </div>
            ...
            {/* Log In button */}
            <button type="submit" className="w-full h-[34px] rounded-[8px] text-white text-[12px] font-medium hover:opacity-90 active:scale-[0.99] transition-all" style={{ background: "#955ac3" }}>
              Log In
            </button>
          </form>
        </div>
      ...
      {authModal === "signup" && (
        <Modal title="Request account access" onClose={() => setAuthModal(null)}>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Student, teacher, parent, and school-admin accounts are provisioned by your school administrator.</p>
            <Input label="Full name" value={signUp.name} onChange={(value) => setSignUp((s) => ({ ...s, name: value }))} required />
            <Input label="Email" type="email" value={signUp.email} onChange={(value) => setSignUp((s) => ({ ...s, email: value }))} required />
            {authNotice && <p className="text-xs text-primary">{authNotice}</p>}
            <Btn onClick={createAccount} className="w-full">
      ...
      {authModal === "reset" && (
        <Modal title="Reset your password" onClose={() => setAuthModal(null)}>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">We will send password reset instructions to your registered email address.</p>
            <Input label="Email" type="email" value={resetEmail} onChange={setResetEmail} required />
            <Btn onClick={requestReset} className="w-full">
              Send reset email
            </Btn>
            {authNotice && <p className="text-xs text-primary">{authNotice}</p>}
          </div>
...
// ─── Super Admin Portal ───────────────────────────────────────────────────────
// ─── Leads mock data ──────────────────────────────────────────────────────────
function RoleSelectionPage({ roles, onSelect }: { roles: Array<{ role: string; school_id: string | null }>; onSelect: (role: string, schoolId: string | null) => void }) {
        ...
        {options.map((option) => (
          ...
          >
            ...
            <div>
              ...
              <p className="mt-3 text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: option.color }}>
                {option.schoolLabel}
              </p>
            </div>
...
const PORTAL_VIEWS: Record<DashboardPortal, string[]> = {
  ...
  teacher: ["dashboard", "classes", "tests", "results", "students", "announcements", "messages", "timetable", "profile"],
...
function roleContextLabel(role: string, schoolName?: string | null) {
  const roleLabel = ROLES.find((item) => item.id === role.replace("_", "-"))?.label ?? role.replace(/_/g, " ")
...
function ActiveContextSwitcher({
  ...
  const uniqueRoles = useMemo(() => Array.from(new Map(roles.map((role) => [`${role.role}::${role.school_id ?? ""}`, role] as const)).values()), [roles])

  useEffect(() => {
    const schoolIds = Array.from(new Set(uniqueRoles.map((role) => role.school_id).filter((schoolId): schoolId is string => Boolean(schoolId))))
    ...
    void supabase
      ...
      .then(({ data }) => {
        if (cancelled) return
        const nextMap = Object.fromEntries(((data as Array<{ id: string; name: string }> | null) ?? []).map((school) => [school.id, school.name]))
  ...
  return (
    <div className="fixed bottom-4 left-4 z-40 w-72 rounded-2xl border border-border bg-card/95 p-3 shadow-xl backdrop-blur">
      <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{language === "ar" ? "سياق العمل" : "Active Context"}</p>
      <p className="mt-1 text-xs text-muted-foreground">{language === "ar" ? "بدّل بين الأدوار أو المدارس المسموح بها." : "Switch between the roles and schools assigned to your account."}</p>
      <select
        value={currentKey}
        onChange={(event) => {
...
function SuperAdminPortal({ view, setView, onLogout, userId }: { view: string; setView: (v: string) => void; onLogout: () => void; userId?: string | null }) {
  ...
  const schools: AdminSchoolRow[] = dbSchools.schools.map((school) => {
    ...
    const preferredPlan = typeof settings.preferred_plan === "string" && settings.preferred_plan.trim() ? settings.preferred_plan : null
    const adminName =
      typeof school.admin_name === "string" && school.admin_name.trim() ? school.admin_name : typeof settings.admin_name === "string" && settings.admin_name.trim() ? settings.admin_name : "-"
    const adminEmail = typeof school.admin_email === "string" && school.admin_email.trim() ? school.admin_email : typeof settings.admin_email === "string" ? settings.admin_email : ""
    ...
    return {
      ...
      subscriptionStatus: school.school_subscriptions?.[0]?.status ?? "unknown",
      renewal: school.school_subscriptions?.[0]?.ends_at ?? null,
  ...
  useEffect(() => {
    if (!dbSchools.loading) {
      setSchools(
        dbSchools.schools.map((s) => {
          const settings = (s.settings ?? {}) as Record<string, unknown>
          const preferredPlan = typeof settings.preferred_plan === "string" && settings.preferred_plan.trim() ? settings.preferred_plan : null
          const adminName = typeof settings.admin_name === "string" && settings.admin_name.trim() ? settings.admin_name : "—"
          return {
            id: s.id,
            name: s.name,
            admin: adminName,
            ...
            subscriptionStatus: s.school_subscriptions?.[0]?.status ?? "unknown",
            renewal: s.school_subscriptions?.[0]?.ends_at ?? null,
  ...
  const saveEdit = async () => {
    ...
    if (selectedPlan) {
      const subscriptionResult = await dbSchools.updateSchoolSubscription(showEdit, selectedPlan.id)
      if (subscriptionResult.error) {
        showToast(subscriptionResult.error, "error")
        return
      }
    }
    setShowEdit(null)
  ...
  const downloadAuditedExport = async (entityType: string, fileName: string, schoolId?: string, filters?: Record<string, unknown>) => {
    const response = await callExportData({ entity_type: entityType, school_id: schoolId, filters })
    if (!response.ok) {
      const body = await response.json().catch(() => ({}))
      showToast(body.error ?? "Could not export the report.", "error")
      return
    }
    ...
    showToast(language === "ar" ? "تم تصدير التقرير وتسجيل العملية بنجاح." : "Report exported and audited successfully.")
  }

  const exportLeadsCSV = () => downloadAuditedExport("waitlist", "nibras-leads.xls")
  ...
  const analyticsPlanData = plans
    .map((plan) => ({ name: plan.name, value: plan.schools }))
    .filter((plan) => plan.value > 0)

  const analyticsRevenueData = Array.from({ length: 6 }, (_, index) => {
    const monthDate = new Date()
    monthDate.setDate(1)
    monthDate.setMonth(monthDate.getMonth() - (5 - index))
    const monthStart = new Date(monthDate.getFullYear(), monthDate.getMonth(), 1)
    const monthEnd = new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0, 23, 59, 59, 999)
    const revenue = dbSchools.schools.reduce((total, school) => {
      const subscription = school.school_subscriptions?.find((item) => {
        const startsAt = new Date(item.starts_at)
        const endsAt = item.ends_at ? new Date(item.ends_at) : null
        return ["active", "trialing"].includes(item.status) && startsAt <= monthEnd && (!endsAt || endsAt >= monthStart)
      })
      return total + ((subscription?.subscription_plans?.price_cents ?? 0) / 100)
    }, 0)
    return {
      month: monthDate.toLocaleDateString(language === "ar" ? "ar-EG" : "en-US", { month: "short", year: "numeric" }),
      revenue,
    }
  })

  const exportAnalyticsReport = () =>
    downloadAuditedExport("platform_analytics", `nibras-platform-analytics-${new Date().toISOString().slice(0, 10)}.xls`)
  ...
  return (
    <>
      <AppShell
        ...
        onSelect={setView}
        onLogout={onLogout}
        headerTitle={titleMap[view] || "Dashboard"}
        ...
        {view === "dashboard" &&
          (platformOverviewLoading ? (
            ...
            <ErrorState
              ...
              description={platformOverviewError}
              onRetry={() => {
                dbSchools.fetchSchools()
                dbLeads.fetchLeads()
              }}
            />
          ) : (
                  ...
                  <tbody>
                    {filteredSchools.map((s) => (
                      <tr key={s.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <Badge color={s.subscriptionStatus === "active" ? "green" : s.subscriptionStatus === "trialing" ? "blue" : "yellow"}>{s.subscriptionStatus}</Badge>
                        </td>
                  ...
                  <tbody>
                    {filteredLeads.map((l) => (
                      <tr key={l.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap" dir="ltr">
                          {l.phone}
                        </td>
                        <td className="px-4 py-3 text-sm text-muted-foreground max-w-[180px] truncate" title={l.email} dir="ltr">
                          {l.email}
                        </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <RPieChart id="chart-2">
                    ...
                    >
                      {analyticsPlanData.map((_, index) => (
                        <Cell key={`plan-cell-${index}`} fill={PLAN_COLORS[index % PLAN_COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip />
                  </RPieChart>
              ...
              <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart
                    id="chart-3"
                    data={analyticsRevenueData}
                  >
                    <XAxis dataKey="month" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${Number(v).toLocaleString()}`} />
                    <Tooltip contentStyle={{ borderRadius: 12, border: "none" }} formatter={(v: number) => [`$${v.toLocaleString()}`, "Revenue"]} />
                    <Bar dataKey="revenue" fill="#7C5CBF" radius={[6, 6, 0, 0]} name="Revenue" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            <div className="flex justify-end">
              <Btn icon={<Download className="w-4 h-4" />} variant="secondary" onClick={exportAnalyticsReport}>
                Export Report
              </Btn>
            </div>
          </div>
                    ...
                    {schools.map((s) => (
                      <tr key={s.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground">
                          {s.renewal ? new Date(s.renewal).toLocaleDateString(language === "ar" ? "ar-EG" : "en-US") : t("Not set")}
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
function StudentPortal({ view, setView, onLogout }: { view: string; setView: (v: string) => void; onLogout: () => void }) {
  ...
  useEffect(() => {
    if (activeTest !== null && !testDone && testSeconds === 0) {
      setTestReview(true)
      showToast("Time is up. Review and submit your test.", "error")
    }
  }, [activeTest, testDone, testSeconds, showToast])
  useEffect(() => {
    if (activeTest === null || testDone) return
  ...
  useEffect(() => {
    if (activeCourseHW !== null && !courseHWDone && courseHWSeconds === 0) {
      setCourseHWReview(true)
      showToast("Time is up. Review and submit your homework.", "error")
    }
  }, [activeCourseHW, courseHWDone, courseHWSeconds, showToast])
  ...
  const startTest = (id: number) => {
    ...
    setTestQ(saved?.question || 0)
    setTestSeconds(saved?.seconds ?? 45 * 60)
    setTestDone(false)
    setTestReview(false)
  }
  const submitTest = () => {
    if (activeTest === null) return
    window.localStorage.removeItem(`nibras-test-draft-${activeTest}`)
    setTestReview(false)
    setTestDone(true)
    setTakenTests((p) => (p.includes(activeTest) ? p : [...p, activeTest]))
  }
  const startCourseHomework = (id: number) => {
    ...
    setActiveCourseHW(id)
    setCourseHWAnswers(saved?.answers || {})
    setCourseHWQ(saved?.question || 0)
    setCourseHWSeconds(saved?.seconds ?? 30 * 60)
    setCourseHWDone(false)
    setCourseHWReview(false)
  }
  const submitCourseHomework = () => {
    if (activeCourseHW === null) return
    window.localStorage.removeItem(`nibras-homework-draft-${selectedCourse}-${activeCourseHW}`)
    setCourseHWReview(false)
    setCourseHWDone(true)
  ...
  const COURSE_LESSONS: Record<string, { id: number; title: string; date: string; duration: string; videoUrl?: string }[]> = {
    ...
    Chemistry: [{ id: 1, title: "Periodic Table Overview", date: "Apr 4, 2024", duration: "45 min" }],
    Biology: [
  ...
  const COURSE_HW: Record<string, { id: number; title: string; qType: string; questions: number; deadline: string; submitted: boolean; score?: number; maxScore: number }[]> = {
    ...
    "English Language": [{ id: 1, title: "Essay: Environmental Impact", qType: "True/False", questions: 4, deadline: "Apr 24, 2024", submitted: true, score: 75, maxScore: 100 }],
    Physics: [{ id: 1, title: "Newton's Laws Quiz", qType: "MCQ", questions: 6, deadline: "Apr 25, 2024", submitted: false, maxScore: 100 }],
    Chemistry: [],
    Biology: [],
  }
  ...
  const COURSE_TASKS: Record<string, { id: number; title: string; qType: string; questions: number; deadline: string; submitted: boolean; score?: number; maxScore: number }[]> = {
    Mathematics: [{ id: 1, title: "Differentiation Exercise", qType: "MCQ", questions: 4, deadline: "Apr 30, 2024", submitted: false, maxScore: 100 }],
    "English Language": [],
    Physics: [],
    Chemistry: [],
    Biology: [],
  ...
  return (
    <>
      <AppShell navItems={STUDENT_NAV} activeView={view} onSelect={setView} onLogout={onLogout} headerTitle={titleMap[view] || "Dashboard"} userName="Joshua Ashiru" userRole="Student – Class A">
            ...
            {["Mathematics", "English Language", "Physics", "Chemistry", "Biology"].map((sub, i) => (
              ...
              >
                <div className="h-28 rounded-xl mb-4 flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${AVATAR_COLORS[i]}18, ${AVATAR_COLORS[i]}05)` }}>
                  <div className="w-16 h-16 rounded-3xl bg-white/85 flex items-center justify-center shadow-sm">
                    <Book className="w-8 h-8" style={{ color: AVATAR_COLORS[i] }} />
                  </div>
                </div>
                ...
                <div className="rounded-lg bg-[#F0E3F8] px-3 py-2 flex items-center justify-between text-xs text-[#0E1B4A]">
                  <span className="flex items-center gap-1.5">
                    <BookOpen className="w-3.5 h-3.5" /> {(COURSE_LESSONS[sub] || []).length}
                  </span>
                  <span className="h-4 w-px bg-[#8C8C8C]/40" />
                  <span className="flex items-center gap-1.5">
                    <ClipboardList className="w-3.5 h-3.5" /> {i + 2}
                  </span>
                  <span className="h-4 w-px bg-[#8C8C8C]/40" />
                  <span className="flex items-center gap-1.5">
            ...
            {courseTab === "lessons" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {(COURSE_LESSONS[selectedCourse] || []).map((lesson, li) => (
                  <button
                    key={lesson.id}
                    onClick={() => setSelectedCourseLesson({ id: lesson.id, title: lesson.title })}
                    className="bg-white rounded-xl border border-[#E7D8F4] shadow-sm overflow-hidden text-left hover:-translate-y-0.5 hover:shadow-md transition-all"
                  >
                    <div className="h-28 bg-gradient-to-br from-[#F8F3FC] to-[#EEE4F5] flex items-center justify-center">
                      <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-sm">
                        <span className="text-lg font-bold text-[#955AC3]">{String(li + 1).padStart(2, "0")}</span>
                      </div>
                    </div>
                    <div className="p-4">
                      ...
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white" style={{ background: "#955AC3" }}>
                        <PlayCircle className="w-3.5 h-3.5" /> Watch lesson
                      </span>
                    </div>
                  </button>
                ))}
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {hw.submitted ? (
                          <div className="text-right shrink-0">
                            <p className="text-xl font-bold" style={{ color: (hw.score || 0) >= 80 ? "#10B981" : "#F59E0B" }}>
                              {hw.score}%
                            </p>
                            <p className="text-[10px] text-[#999]">
                              {hw.score}/{hw.maxScore} pts
                            </p>
                          </div>
                        ) : (
                          <button onClick={() => startCourseHomework(hw.id)} className="px-4 py-2 rounded-lg text-white text-xs font-semibold shrink-0" style={{ background: "#955AC3" }}>
                            Start
                          </button>
                        )}
                      </div>
              ...
              <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-6">
                          <div>
                            <p className="text-2xl font-bold text-[#955AC3]">
                              {g.score}/{g.maxScore}
                            </p>
                            <p className="text-xs text-[#999]">Score / Max</p>
                          </div>
                          <div>
                            <p className="text-2xl font-bold text-[#0E1B4A]">{g.gpa}</p>
                            <p className="text-xs text-[#999]">GPA</p>
                          </div>
                          <div>
                            <span className="px-3 py-1 rounded-full text-sm font-bold bg-emerald-100 text-emerald-700">{g.grade}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-50">
                    {[
                      { label: "Homework Average", score: 85, max: 100 },
                      { label: "Monthly Test", score: 88, max: 100 },
                      { label: "Final Exam", score: 90, max: 100 },
                    ].map((item) => (
                      <div key={item.label} className="px-5 py-4 flex items-center gap-4">
                        ...
                        <span className="text-sm font-bold text-[#0E1B4A] w-16 text-right">
                          {item.score}/{item.max}
                        </span>
                      </div>
                    ))}
            ...
            <div className="max-w-2xl bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-center justify-between mb-5">
                <span className="text-sm font-semibold text-[#999]">
                  Question {courseHWQ + 1} of {mockQuestions.length}
                </span>
                <span className="text-xs font-bold text-[#955AC3]">
                  {formatTime(courseHWSeconds)} · Draft saved {draftSavedAt || "now"}
                </span>
              ...
              <div className="space-y-3">
                {mockQuestions[courseHWQ].opts.map((opt, i) => (
                  <button
                    key={i}
                    onClick={() => setCourseHWAnswers((p) => ({ ...p, [courseHWQ]: opt }))}
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${courseHWAnswers[courseHWQ] === opt ? "border-[#955AC3] bg-[#955AC3]" : "border-gray-300"}`}
                    >
              ...
              <div className="flex items-center justify-between mt-6">
                <button
                  onClick={() => setCourseHWQ((q) => Math.max(0, q - 1))}
                  disabled={courseHWQ === 0}
                ...
                {courseHWQ < mockQuestions.length - 1 ? (
                  ...
                  <button onClick={() => setCourseHWReview(true)} className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "#955AC3" }}>
                    Submit Homework
                  </button>
                )}
              </div>
        ...
        {view === "courses" && selectedCourse && activeCourseHW !== null && !courseHWDone && courseHWReview && (
          <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-[#E7D8F4] shadow-sm p-6 space-y-5">
            <div>
              <h2 className="text-lg font-bold text-[#0E1B4A]">Review homework</h2>
              <p className="text-sm text-[#999]">
                {Object.keys(courseHWAnswers).length} of {mockQuestions.length} questions answered. You can return and edit before final submission.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {mockQuestions.map((_, index) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-sm font-semibold ${courseHWAnswers[index] ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}
                >
                  Question {index + 1}: {courseHWAnswers[index] ? "Answered" : "Missing"}
                </button>
              ))}
            </div>
            <div className="flex justify-between">
              <Btn variant="secondary" onClick={() => setCourseHWReview(false)}>
                Back to edit
              </Btn>
              <Btn onClick={submitCourseHomework}>Submit homework</Btn>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 bg-white rounded-xl border border-gray-200 px-3 py-2">
                ...
                <select value={testCourseFilter} onChange={(e) => setTestCourseFilter(e.target.value)} className="text-sm font-medium text-[#0E1B4A] bg-transparent outline-none cursor-pointer">
                  ...
                  {["Mathematics", "English Language", "Physics", "Chemistry", "Biology"].map((c) => (
                ...
                <div key={t.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {takenTests.includes(t.id) ? (
                      ...
                      <button onClick={() => startTest(t.id)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold shrink-0" style={{ background: "#955AC3" }}>
          ...
          <div className="max-w-2xl mx-auto space-y-5">
            <div className="flex items-center justify-between bg-card rounded-xl p-3 border border-border shadow-sm">
              ...
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-sm font-bold">{formatTime(testSeconds)}</span>
                <span className="text-[10px] text-muted-foreground">Draft saved {draftSavedAt || "now"}</span>
              </div>
            </div>
            <div className="bg-card rounded-2xl p-6 border border-border shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm font-semibold text-muted-foreground">
                  Question {testQ + 1} of {mockQuestions.length}
                </span>
              ...
              <div className="space-y-3">
                {mockQuestions[testQ].opts.map((opt, i) => (
                  <button
                    key={i}
                    onClick={() => setTestAnswers((p) => ({ ...p, [testQ]: opt }))}
              ...
              <div className="flex items-center justify-between mt-6">
                <Btn variant="secondary" onClick={() => setTestQ((q) => Math.max(0, q - 1))} disabled={testQ === 0} icon={<ChevronLeft className="w-4 h-4" />}>
                  Previous
                </Btn>
                {testQ < mockQuestions.length - 1 ? (
                  <Btn onClick={() => setTestQ((q) => q + 1)} icon={<ChevronRight className="w-4 h-4" />}>
                    Next
                  </Btn>
                ) : (
        ...
        {view === "tests" && activeTest !== null && !testDone && testReview && (
          <div className="max-w-2xl mx-auto bg-card rounded-2xl p-6 border border-border shadow-sm space-y-5">
            <div>
              <h2 className="text-lg font-bold text-foreground">Review test answers</h2>
              <p className="text-sm text-muted-foreground">
                {Object.keys(testAnswers).length} of {mockQuestions.length} questions answered. Unanswered questions are marked for review.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {mockQuestions.map((_, index) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-sm font-semibold ${testAnswers[index] ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}
                >
                  Question {index + 1}: {testAnswers[index] ? "Answered" : "Missing"}
                </button>
              ))}
            </div>
            <div className="flex justify-between">
              <Btn variant="secondary" onClick={() => setTestReview(false)}>
                Back to edit
              </Btn>
              <Btn onClick={submitTest}>Submit test</Btn>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5" style={{ fontFamily: "'Poppins', sans-serif" }}>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-[#999]">Cumulative GPA</p>
                  <p className="text-xl font-bold text-[#0E1B4A]">3.80</p>
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-[#999]">Class Rank</p>
                  <p className="text-xl font-bold text-[#0E1B4A]">#2</p>
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-[#999]">Distinctions</p>
                  <p className="text-xl font-bold text-[#0E1B4A]">2 Courses</p>
                </div>
              </div>
            </div>
            {/* Final Grades table */}
                  ...
                  <thead>
                    <tr className="bg-[#F7F4FF]">
                      {["Course", "Score", "Max Score", "GPA", "Grade"].map((h) => (
                        <th key={h} className="text-left px-6 py-3 text-xs font-semibold text-[#0E1B4A]">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {FINAL_GRADES_DATA.map((g) => (
                      <tr key={g.course} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-sm font-bold" style={{ color: parseFloat(g.gpa) >= 3.7 ? "#10B981" : parseFloat(g.gpa) >= 3.0 ? "#955AC3" : "#F59E0B" }}>
                            {g.gpa}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`text-[11px] px-2.5 py-1 rounded-full font-semibold ${g.grade.startsWith("A") ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>
                            {g.grade}
                          </span>
                        </td>
                      </tr>
              ...
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  {Object.keys(TIMETABLE).map((d) => (
                    <th key={d} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">
                      {d}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={i} className="border-b border-border last:border-0">
                    ...
                    {Object.values(TIMETABLE).map((days, di) => (
                      <td key={di} className="px-4 py-3">
                        {days[i] === "Break" ? (
                          <span className="text-xs bg-amber-50 text-amber-700 px-2 py-1 rounded-lg font-medium">{t("Break")}</span>
                        ) : days[i] === "Free" ? (
                          <span className="text-xs text-muted-foreground">{t("Free")}</span>
                        ) : (
                          <div className="bg-secondary rounded-lg px-2 py-1.5">
                            <p className="text-xs font-semibold text-primary">{t(days[i])}</p>
                          </div>
                        )}
                      </td>
                    ))}
              ...
              <div key={hw.id} className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Submitted on {hw.due} · {hw.class}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-primary">{80 + i * 5}%</p>
                    <p className="text-xs text-muted-foreground">
                      {(hw.points * (80 + i * 5)) / 100}/{hw.points} pts
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <LanguageProvider>
      <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        {!auth.loading && !isResetRoute && auth.portal === "login" && <AuthPage onLogin={auth.signIn} onSignUpSchool={auth.signUpSchool} authError={auth.error} />}
        ...
        {!auth.loading && !isResetRoute && auth.portal === "school-admin" && (
          <Suspense fallback={null}>
            <SchoolAdminPortal view={view} setView={handleViewChange} onLogout={handleLogout} schoolId={auth.activeSchoolId} user={auth.user} />
          </Suspense>
        )}
        {!auth.loading && !isResetRoute && auth.portal === "teacher" && (
          <Suspense fallback={null}>
            <TeacherPortal view={view} setView={handleViewChange} onLogout={handleLogout} schoolId={auth.activeSchoolId} user={auth.user} />
          </Suspense>
        )}
        {!auth.loading && !isResetRoute && auth.portal === "student" && (
          <Suspense fallback={null}>
            <StudentPortalLive view={view} setView={handleViewChange} onLogout={handleLogout} schoolId={auth.activeSchoolId} user={auth.user} />
          </Suspense>
        )}
        {!auth.loading && !isResetRoute && auth.portal === "parent" && (
          <Suspense fallback={null}>
            <ParentPortalLive view={view} setView={handleViewChange} onLogout={handleLogout} schoolId={auth.activeSchoolId} user={auth.user} />
          </Suspense>
        )}
        {!auth.loading && !isResetRoute && auth.user && auth.roles.length > 1 && (
          <ActiveContextSwitcher roles={auth.roles} activeRole={auth.activeRole} activeSchoolId={auth.activeSchoolId} onSwitch={handleContextSwitch} />
        )}
      </div>
    </LanguageProvider>
  )
